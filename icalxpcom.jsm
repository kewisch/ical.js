...
const EXPORTED_SYMBOLS = ["defineXPCOMProps", "forwardAttribute", "forwardFunc", "unwrapFunc", "forwardInnerAttribute", "forwardInnerFunc", "unwrapValue"];
  ...
  proto[name] = function () {
    ...
    if(unwrapIndices) {
      ...
      for each(let index in unwrapIndices) {
        ...
        if(newArgs[index] && "wrappedJSObject" in newArgs[index]) {
          ICAL.helpers.dumpn("  to  " + newArgs[index].wrappedJSObject.mInner + " what about " + newArgs[index].wrappedJSObject);
