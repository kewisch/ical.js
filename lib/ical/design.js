...
const vcardValues = extend(commonValues, {
  ...
  timestamp: {
    ...
    // RFC 6350 only allows the basic format for a timestamp, but the extended
    // format is widely used. The iCalendar conversions insert and remove the
    // separators at fixed offsets, which mangles a value that already is in
    // the extended format, so use the date-and-or-time conversions, which
    // accept both.
    fromICAL: function(aValue) {
      return vcardValues['date-and-or-time'].fromICAL(aValue);
    },
