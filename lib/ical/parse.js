ICAL.parse = (function() {
  ...
  parser._handleContentLine = function(line, state) {
    ...
    if (paramPos !== -1) {
      ...
      parsedParams = parser._parseParameters(line, paramPos);
      params = parsedParams[0];
      parsedValue = parsedParams[1];
      var lastParamIndex = parsedValue.length + parsedParams[2];
      // if valuePos is between the parameter value we need to adjust.
      if (valuePos < lastParamIndex) {
        valuePos = lastParamIndex +
          line.substr(lastParamIndex).indexOf(VALUE_DELIMITER);
      }
      if (valuePos !== -1) {
