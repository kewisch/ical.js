...
parse._handleContentLine = function(line, state) {
  ...
  if (paramPos !== -1) {
    ...
    let paramString = line.slice(Math.max(0, paramPos));

    // For vCard 2.1, convert bare parameters (e.g., TEL;WORK;VOICE) to explicit TYPE format
    if (state.designSet && state.designSet.name === "vcard21") {
      paramString = parse._normalizeVCard21Params(paramString);
    }

    parsedParams = parse._parseParameters(paramString, 0, state.designSet);
    if (parsedParams[2] == -1) {
    ...
    // Use paramString instead of line for finding the value when parameters were normalized
    let lastParamIndexInParamString = parsedParamLength + parsedParams[2];
    if ((lastValuePos =
      paramString.slice(Math.max(0, lastParamIndexInParamString)).indexOf(VALUE_DELIMITER)) !== -1) {
      value = paramString.slice(Math.max(0, lastParamIndexInParamString + lastValuePos + 1));
    } else {
...
/**
 * Normalize vCard 2.1 parameters by converting bare parameter values to TYPE= format.
 * For example, converts "TEL;WORK;VOICE" to "TEL;TYPE=WORK;TYPE=VOICE"
 *
 * @function ICAL.parse._normalizeVCard21Params
 * @private
 * @param {String} paramString        The parameter string starting with ';'
 * @return {String}                   Normalized parameter string
 */
parse._normalizeVCard21Params = function(paramString) {
  // Split by semicolon, keeping track of the colon position for the value
  let colonPos = paramString.indexOf(':');
  if (colonPos === -1) {
    return paramString;
  }

  let paramsOnly = paramString.slice(0, colonPos);
  let valueAndRest = paramString.slice(colonPos);

  // Split parameters by semicolon
  let segments = paramsOnly.split(';');
  let normalizedSegments = [];

  for (let i = 0; i < segments.length; i++) {
    let segment = segments[i];
    if (segment === '') {
      // Empty segment (from leading ';'), keep it
      normalizedSegments.push(segment);
      continue;
    }

    // Check if this segment contains '='
    if (segment.indexOf('=') !== -1) {
      // Already in name=value format, keep as is
      normalizedSegments.push(segment);
    } else {
      // Bare parameter value - convert to TYPE=value format
      normalizedSegments.push('TYPE=' + segment);
    }
  }

  return normalizedSegments.join(';') + valueAndRest;
};
