  ...
  ICAL.Time.prototype = {
    ...
    adjust: function icaltime_adjust(aExtraDays, aExtraHours,
      ...
      if (!time.isDate) {
        ...
        minute = time.minute + aExtraMinutes + minutes_overflow;
        ...
        hour = time.hour + aExtraHours + hours_overflow;
        time.hour = hour % 24;
      ...
      if (time.month > 12) {
        years_overflow = ICAL.helpers.trunc((time.month - 1) / 12);
      } else if (time.month < 1) {
        years_overflow = ICAL.helpers.trunc(time.month / 12) - 1;
      }

      time.year += years_overflow;
      time.month -= 12 * years_overflow;

      // Now take care of the days (and adjust month if needed)
      day = time.day + aExtraDays + days_overflow;
      if (day > 0) {
        for (;;) {
    ...
    function defineAttr(attr) {
      Object.defineProperty(ICAL.Time.prototype, attr, {
        get: function getTimeAttr() {
          if (this._needsNormalize) {
            this._normalize();
            this._needsNormalize = false;
          }

          return this._time[attr];
        },
        set: function setTimeAttr(val) {
          this._needsNormalize = true;
          this._time[attr] = val;
          return val;
        }
      });
