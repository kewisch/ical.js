ICAL.Serializerv2 = (function() {
  ...
  var Serailize = {
    ...
    serializeToIcal: function(jCal) {
      ...
      for (; i < len; i++) {
        result += Serailize.stringifyComponent(jCal[i]) + LINE_ENDING;
      }
     ...
     */
    stringifyComponent: function(component) {
      ...
      for (; propIdx < propLen; propIdx++) {
        result += Serailize.stringifyProperty(props[propIdx]) + LINE_ENDING;
      }
      ...
      for (; compIdx < compLen; compIdx++) {
        result += Serailize.stringifyComponent(comps[compIdx]) + LINE_ENDING;
      }
     ...
     */
    stringifyProperty: function(property) {
      ...
      for (paramName in params) {
        if (params.hasOwnProperty(paramName)) {
          ...
          line += '=' + Serailize.stringifyPropertyValue(params[paramName]);
        }
      }
      ...
      if (multiValue) {
        line += Serailize.stringifyMultiValue(
          property.slice(3), multiValue, valueType
        );
      } else {
        line += Serailize.stringifyValue(property[3], valueType);
      }
     ...
     */
    stringifyPropertyValue: function(value) {
     ...
     */
    stringifyMultiValue: function(values, delim, type) {
      var result = '';
      var len = values.length;
      var i = 0;

      for (; i < len; i++) {
        result += Serailize.stringifyValue(values[i], type);
        if (i !== (len - 1)) {
          result += delim;
        }
      }
     ...
     */
    stringifyValue: function(value, type) {
