...
ICAL.helpers = {
  ...
  indexes: function(buffer, search, pos, end) {

    var result = [];
    buffer = buffer.substr(pos,end);
    while ( pos < end ) {
  ...
  firstIndexOf: function(buffer, search)  {
    var result = [];
    var pos = 0;
    var str1 = '';
    while (buffer[pos] !== search)  {
    ...
    result = [str1, buffer.substring(pos),pos];
    return result;
  },

  splitOnFirst: function(line, index)  {
    return [line.substr(0,index), line.substring(index)];
  },

  specialIndexOf: function()  {
    while ((pos = buffer.indexOf(search, pos)) !== -1) {
      if (buffer[pos - 1] === '\\') {
        pos += 1;
      } else {
