...
class RecurIterator {
  ...
  init() {
    ...
    if (this.rule.freq == "WEEKLY") {
      if ("BYDAY" in parts) {
        let [pos, dow] = this.ruleDayOfWeek(parts.BYDAY[0], this.rule.wkst);
  ...
  next_day() {
    let has_by_day = ("BYDAY" in this.by_data);
  ...
  next_week() {
    ...
    if (this.has_by_data("BYWEEKNO")) {
      let idx = ++this.by_indices.BYWEEKNO;
  ...
  next_month() {
    let this_freq = (this.rule.freq == "MONTHLY");
    let data_valid = 1;
  ...
  next_generic(aRuleType, aInterval, aDateAttr, aFollowingAttr, aPreviousIncr) {
    ...
    if (has_by_rule) {
      ...
      let idx = this.by_indices[aRuleType];
