...
(function() {
  ...
  ICAL.icalrecur.prototype = {
    ...
    fromData: function fromData(aData) {
      ...
      if (this.until) {
        if (this.until instanceof Time) {
          this.until = this.until.clone();
        } else {
          this.until = Time.fromData(this.until);
        }
      }
  ...
  icalrecur_iterator.prototype = {
    ...
    init: function icalrecur_iterator_init() {
      ...
      if (this.rule.freq == "YEARLY") {
        ...
        var next = Time.fromDayOfYear(this.days[0], this.last.year);
     ...
     * Takes into account order (will sort the results)
     * and negative by day rules
     *
    ...
    normalizeByMonthDayRules: function(year, month, rules) {
      ...
      for(; ruleIdx < len; ruleIdx++) {
        ...
        //XXX: Its possible to have a value of zero?
        //     remove those values maybe?
    ...
    _byDayAndMonthDay: function(isInit) {
      /**
       * We are given a list of dates in the month (23, etc..)
       * Also we are given a list of days (MO, 2SU, etc..) when
       * both conditions match the day the next item.
       *
       * To find the next item we begin at dtstart.
       * Iterate through each date and see a corresponding
       * day matches.
       */
      ...
      while (!data_valid) {
        ...
        if (!data_valid && dateIdx === (dateLen - 1)) {
          nextMonth();
          continue;
        }
      }
    ...
    next_weekday_by_week: function next_weekday_by_week() {
      ...
      for (;;) {
        ...
        var next = Time.fromDayOfYear(startOfWeek + dow,
                                                  this.last.year);
    ...
    next_year: function next_year() {
      ...
      var next = Time.fromDayOfYear(this.days[this.days_index],
                                                this.last.year);
